...
public partial class Customer_CustomerInfoUpdate : System.Web.UI.Page
{
    ...
    string strLogo = "http://khachhang.sucmanhcong.com/Images/Logo.png";
    ...
    public void setCustomer()
    {
        try
        {
            ...
            {
                this.strMsg = "Bạn chưa nhập ngày sinh ";
           ...
           if (upImage1.PostedFile.FileName != "")
            ...
            try
            {
                DateTime datee = DateTime.ParseExact(this.txtBirthday.Text, "dd/MM/yyyy", DateTimeFormatInfo.InvariantInfo);
                Cmd.Parameters.Add("Birthday", SqlDbType.DateTime).Value = datee;
            }
            catch
            {
                Page.ClientScript.RegisterStartupScript(this.GetType(), "showMsg", "alert('Ngày sinh không hợp lệ');", true);
            ...
            Cmd.Parameters.Add("Avatar", SqlDbType.NVarChar).Value = (this.txtAvatar.Text != "" ? "http://khachhang.sucmanhcong.com/Images/Customer/" + this.txtAvatar.Text : strLogo);
            Cmd.ExecuteNonQuery();
            sqlCon.Close();
            sqlCon.Dispose();
            this.strMsg = "Đã cập nhật  !";
            Page.ClientScript.RegisterStartupScript(this.GetType(), "showMsssg", "if(confirm('Hồ sơ đả được cập nhật ')) {window.location.reload(false);}", false); // alert('Thông tin đã được cập nhật thành công 
            this.getCustomer();
            return;
        }
        catch (Exception Ex)
    ...
    private bool saveImage1()
    {
        ...
        try
        {
            ...
            else
            {
                ...
                this.lblImg1.Text = "<img width = \"125px\" height = \"100px\"  style=\"margin-left:-20px;\" src = \"../Images/Customer/" + sFileName + strEx + "\">";
                return true;
            }
        }
        catch (Exception ex)
