...
public class Customers
{
    ...
    #region updateCard
    public int updateCard(string customerAccount,string typeCard)
    {
        try
        {
            SqlConnection sqlCon = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["TVSConn"].ConnectionString);
            sqlCon.Open();
            SqlCommand Cmd = sqlCon.CreateCommand();
            Cmd.CommandText = "UPDATE tblCustomers SET  AccountType = @AccountType  WHERE  Account = @CustomerAccount";
            Cmd.Parameters.Add("CustomerAccount", SqlDbType.NVarChar).Value = customerAccount;
            Cmd.Parameters.Add("AccountType", SqlDbType.NVarChar).Value = typeCard;
            int ret = Cmd.ExecuteNonQuery();

            double subMoney = 0;
            if (typeCard == "CustomerAccount1") subMoney = 360000;
            else if (typeCard == "CustomerAccount2") subMoney = 3600000;
            else
            { return 0; }

            SqlCommand Cmd2 = sqlCon.CreateCommand();
            Cmd2.CommandText = "INSERT INTO tblPartnerBill(CustomerAccount,PartnerAccount,TotalMoney,TotalPeyment,Note) VALUES (@CustomerAccount,'SMC-CARD',@TotalMoney,@TotalPeyment,'NANG CAP THE')";     // THêm 1 cột với giá trị âm
            ...
            Cmd2.Parameters.Add("TotalPeyment", SqlDbType.Float).Value = (subMoney); // 2 cột có giá trị như nhau có ảnh hưởng gì đến những tính toán khác hay không ?/

            ret = Cmd2.ExecuteNonQuery();
      
             SqlCommand Cmd3 = sqlCon.CreateCommand();
             Cmd3.CommandText = "INSERT INTO tblCustomersPaymentByCard (BillId,TotalMoney,TotalPeymentCard,CustomerAccount)  VALUES ( (Select max(BillId) from tblCustomersPaymentByCard ) + 1 ,@TotalMoney,@TotalPeymentCard, @CustomerAccount)";     // THêm 1 cột với giá trị âm
            Cmd3.Parameters.Add("CustomerAccount", SqlDbType.NVarChar).Value = customerAccount;
            Cmd3.Parameters.Add("TotalMoney", SqlDbType.Float).Value = (subMoney);         // <- here
            Cmd3.Parameters.Add("TotalPeymentCard", SqlDbType.Float).Value = (subMoney);          // 2 cột có giá trị như nhau 
